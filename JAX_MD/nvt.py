...
field_gpu = jnp.array(np.array(Field).astype(float_dtype))
...
def energy_fn(positions, neighlist=neighlist):
    ...
    return model.apply(params, positions, field_gpu, cell_gpu, disp_cell, jax.lax.stop_gradient(cut_neighlist), jax.lax.stop_gradient(cut_shifts), center_factor, species)
