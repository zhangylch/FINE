...
from ASE.nemp import sparse_tp
from ASE.nemp.data_config import ModelConfig
from ASE.nemp import MLP
...
class MPNNCore(nnx.Module):
    """
    NNX implementation of the single-graph ASE model.
    """
    ...
    def __call__(self, cart, field, cell, disp_cell, neighlist, shiftimage, center_factor, species):
        ...
        node_field_intensity = jnp.broadcast_to(field_intensity[0], (numatom,))
        sph_field_node = jnp.broadcast_to(sph_field[0], (numatom, sph_field.shape[1]))
        ...
        field_coeff = self.FieldNN_init(field_input).reshape(numatom, prmaxl_i, nwave_i)
        ...
        for iter_loop in range(self.config.MP_loop):
            ...
            field_coeff = self.FieldNN_list[iter_loop](field_input).reshape(numatom, rmaxl_i, nwave_i)
            field_coeff = field_coeff / jnp.sqrt(dtype_2)
    ...
    def sum_interaction(self, numatom, prmaxl_i, field_orbital, center_orbital, contract_coeff, tp_layer, spec_indices, orb_coeff, neighlist, inv_ave_neigh, pindex_l, sph, dtype_2):
